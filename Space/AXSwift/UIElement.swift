...
open class UIElement {
    ...
    open func attributes() throws -> [AXAttribute] {
        let attrs = try attributesAsStrings()
        for attr in attrs where AXAttribute(rawValue: attr) == nil {
            print("Unrecognized attribute: \(attr)")


        }
        return attrs.compactMap({ AXAttribute(rawValue: $0) })
    }
    ...
    /// can use this method.
    open func attributeIsSupported(_ attribute: AXAttribute) throws -> Bool {
    ...
    open func attributeIsSettable(_ attribute: AXAttribute) throws -> Bool {
    ...
    open func attribute<T>(_ attribute: AXAttribute) throws -> T? {
    ...
    open func setAttribute(_ attribute: AXAttribute, value: Any) throws {
    ...
    open func getMultipleAttributes(_ names: AXAttribute...) throws -> [AXAttribute: Any] {
        return try getMultipleAttributes(names)
    }

    open func getMultipleAttributes(_ attributes: [AXAttribute]) throws -> [AXAttribute: Any] {
        let values = try fetchMultiAttrValues(attributes.map({ $0.rawValue }))
        return try packMultiAttrValues(attributes, values: values)
    }

    func getMultipleAttributes(_ attributes: [AXAttribute]) throws -> [AXAttribute: String] {
        let values = try fetchMultiAttrValues(attributes.map({ $0.rawValue }))
        var result = [AXAttribute: String]()
        for (index, attribute) in attributes.enumerated() {
            if try checkMultiAttrValue(values[index]) {
                result[attribute] = String(describing: unpackAXValue(values[index]))
            }
        }
        return result
    }
    ...
    open func arrayAttribute<T>(_ attribute: AXAttribute) throws -> [T]? {
    ...
    (_ attribute: AXAttribute, startAtIndex index: Int, maxValues: Int) throws -> [T]? {
    ...
    open func valueCountForAttribute(_ attribute: AXAttribute) throws -> Int? {
    ...
    open func parameterizedAttributes() throws -> [AXAttribute] {
        return try parameterizedAttributesAsStrings().compactMap({ AXAttribute(rawValue: $0) })
    }
    ...
    /// for more info.
    open func parameterizedAttribute<T, U>(_ attribute: AXAttribute, param: U) throws -> T? {
...
extension UIElement: CustomStringConvertible {
    ...
    public var inspectDict: [AXAttribute: String] {
        guard let attributeNames = try? attributes() else {
            return [:]
        }
        guard let attributes = try? getMultipleAttributes(attributeNames) else {
